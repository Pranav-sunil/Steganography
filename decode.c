...
/* File contains function definitions for carrying out
   decoding process to obtain the secret file */

Status open_file_decode( DecodeInfo *decInfo )
{
	//Function opens the source image file in read mode
...
{
	/* Function checks if the file passed by the user is 
	   a bmp file or not.
	   It also checks if the user has passed output file name */
...
{
	/* Function calls other functions to do the decpding process */
	...
	if( open_file_decode( decInfo ) == e_success )
	{
		...
		{
			printf("INFO: Decoding Magic String Signature Length\n");
			...
			{
				printf("ERROR: Unable to fetch Magic String Length\n");
				return e_failure;
			}
		}
		else
...
{
	//This function fetches 4 byte data from RGB data of source image
	//Size stored as long and returned to the caller
	long size=0;
	int k=31, i=0;
	for( i = 0 ; i < 32 ; i++ )
	{
...
{
	/* Function decodes the length of the magic string
	...
	decInfo->magic_str_length_file = decode_size_from_image( buffer);
	return e_success;
}

Status decode_magic_string( DecodeInfo *decInfo )
{
	/* Decodes the data from image and obtains the magic string and
	   checks if the obtained magic string is same as the 
	    original magic string signature */
	int i;
	char buffer[8];
	for( i = 0 ; i < (int)(decInfo->magic_str_length_file) ; i++ )
	{
		...
		decInfo->magic_string_sig_file[i]=decode_data_from_image( buffer); //Stores the magic string character
	}
	decInfo->magic_string_sig_file[i]='\0';
	if( strcmp(	decInfo->magic_string_sig_file, decInfo->magic_string_sig_user) != 0 )
	{
...
{
	/* Fucntion obtains 8 byte of data from source image and decodes it
